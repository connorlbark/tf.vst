...
if %1 == 1 (echo Making tf_vst Windows DEMO VERSION distribution ...) else (echo Making tf_vst Windows FULL VERSION distribution ...)
...
REM - msbuild tf_vst-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild tf_vst.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild tf_vst.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\tf_vst.aaxplugin\Contents\Win32\tf_vst.aaxplugin --out .\build-win\aax\bin\tf_vst.aaxplugin\Contents\Win32\tf_vst.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\tf_vst.aaxplugin\Contents\x64\tf_vst.aaxplugin --out .\build-win\aax\bin\tf_vst.aaxplugin\Contents\x64\tf_vst.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\tf_vst.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\tf_vst.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "tf_vst Installer" ".\installer\tf_vst Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\tf_vst Installer.exe" ".\installer\tf_vst Demo Installer.exe"
REM -del ".\installer\tf_vst Installer.exe"
REM -)
